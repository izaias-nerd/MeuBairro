...
function displayData(rows) {
    ...
    rows.forEach(row => {
        ...
        card.innerHTML = `
            ...
            <button class="status-button ${status === 'livre' ? 'green' : 'red'}" data-nome="${nome}" data-sobrenome="${sobrenome}" data-status="${status}" onclick="toggleStatus(event)">${status === 'livre' ? 'Sinalizar Ocupado' : 'Sinalizar Livre'}</button>
        `;
